...
async def entry_point_type_scan(chunk: list, **kwargs) -> list:
    ...
    if _extract is False:
        return [await type_scan(item, _recognized_files, _buffer_max, _type_suffix, _target, _program_root,
                                _digits, _human_size) for item in chunk]
...
async def extract_type_scan(_file: str, _buffer_max: int, _recognized_files: list,
    ...
    _results = [await type_scan(_file, _recognized_files, _buffer_max, _type_suffix, _target, _program_root,
                                _digits, _human_size)]
    ...
    if result_bool is True:
        ...
        for sub_file in sub_files:
            res = await type_scan(sub_file, _recognized_files, _buffer_max, _type_suffix, _target, _program_root,
                                  _digits, _human_size)
            if res is not None:
                if len(res) == 4:
