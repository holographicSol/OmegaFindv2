...
async def scan_check(file: str, suffix: str, buffer: bytes, _recognized_files: list, _digits=True) -> list:
    sub_buffer = buffer
    if _digits is False:
...
async def type_scan_check(file: str, buffer: bytes, _recognized_files: list, _digits=True) -> list:
    sub_buffer = buffer
    if _digits is False:
        sub_buffer = await asyncio.to_thread(handler_strings.sub_str, _buffer=buffer)  # digitless
