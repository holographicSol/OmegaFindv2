        ...
        for _result in _results:
            ...
            if check_ not in recognized_files:
                if check_ not in filtered_results:
                    filtered_results.append(_result)
    except:
        pass


async def async_de_obfuscate(_results: list, _recognized_files: list) -> list:
    global recognized_files
    global filtered_results
    recognized_files = _recognized_files
    filtered_results = []
    for result in _results:
        await asyncio.gather(*map(search, [[result]]))
